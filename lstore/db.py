...
class LockManager:
    ...
    def acquire_lock(self, transaction_id, record_id, operation):
        ...
        with self.mutex:
            ...
            print(f"LockManager: Lock type for {operation} is {lock_type}")

            # Initialize lock state if the record is not already locked
            if record_id not in self.locks:
                self.locks[record_id] = (set(), None)
                print(f"LockManager: No existing locks on record {record_id}")

            shared_lock_tids, exclusive_lock_tid = self.locks[record_id]
            print(f"LockManager: Current locks on record {record_id}: shared by {shared_lock_tids}, exclusive by {exclusive_lock_tid}")
            ...
            if lock_type == "shared":
                ...
                if exclusive_lock_tid is None or exclusive_lock_tid == transaction_id:
                    ...
                    return True
                print(f"LockManager: Denied shared lock to transaction {transaction_id} on record {record_id} - exclusive lock held by {exclusive_lock_tid}")
                return False
                ...
                if (not shared_lock_tids and exclusive_lock_tid is None) or \
                    ...
                    return True
                print(f"LockManager: Denied exclusive lock to transaction {transaction_id} on record {record_id} - conflicting locks exist")
                return False
